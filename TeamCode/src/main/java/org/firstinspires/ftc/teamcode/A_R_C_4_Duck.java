...
@Autonomous(name = "A_R_C_4_Duck", group = "Auton")
// @Autonomous(...) is the other common choice

public class A_R_C_4_Duck extends OpMode {
    ...
    public void init() {
        ...
        robot.init();
        telemetry.addData("A_R_C_4_Duck", "Initialized");
        robot.cmd_Set_Required_Hit_Count(2);
        robot.sensor_arm.Set_Read_Position(.6);
    }
    ...
    public void loop() {
        ...
        if (currentStage == stage._78_BACK_UP) {
            if (robot.driveTrain.getcmdComplete()) {
                robot.driveTrain.cmdDriveByGyro(-AUTO_DRIVE_SLOW_SPEED, 90, -0); // Of 32.25
                currentStage = stage._82_BACK_UP;
            }
        }
        ...
        if (currentStage == stage._82_BACK_UP) {
             if (robot.driveTrain.getcmdComplete()) {
                robot.driveTrain.cmdDriveByGyro(-AUTO_DRIVE_SLOW_SPEED, 90, -6.5);// 9 Of 32.25
                robot.cmd_Reset_Hit_Count();
        ...
        if (currentStage == stage._83_BACK_UP) {
            ...
            if (robot.driveTrain.getcmdComplete()) {
                robot.driveTrain.cmdDriveByGyro(-AUTO_DRIVE_SLOW_SPEED, 90, -3.5); // 13 Of 32.25
                currentStage = stage._86_BACK_UP;
            }
        }
        if (currentStage == stage._86_BACK_UP) {
            if (robot.driveTrain.getcmdComplete()) {
                robot.driveTrain.cmdDriveByGyro(-AUTO_DRIVE_SLOW_SPEED, 90, -6); // 18 Of 32.25
                robot.cmd_Reset_Hit_Count();
        ...
        if (currentStage == stage._87_BACK_UP) {
            if (FOUNDTE == false) {
                FOUNDTE = robot.cmd_Set_Delivery_By_Sensor_Short(Sensor_Arm.Side.LEFT, Delivery.Mode.MIDDLE);
