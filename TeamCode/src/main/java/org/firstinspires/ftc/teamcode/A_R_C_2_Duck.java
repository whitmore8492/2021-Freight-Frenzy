...
@Autonomous(name = "A_R_C_2_Duck", group = "Auton")
// @Autonomous(...) is the other common choice

public class A_R_C_2_Duck extends OpMode {
    ...
    public void init() {
        ...
        robot.init();
        telemetry.addData("A_R_C_2_Duck", "Initialized");
    }
    ...
    public void loop() {
        ...
        if (currentStage == stage._45_Back_Up){
            if (runtime.milliseconds() > 4000) {
                robot.driveTrain.cmdDriveByGyro(-AUTO_DRIVE_SLOW_SPEED, 45, -1.0);
        ...
        if (currentStage == stage._80_Back_Up){
            if (robot.driveTrain.getcmdComplete()){
                robot.driveTrain.cmdDriveByGyro(AUTO_DRIVE_SLOW_SPEED, 90, -24);
        ...
        if(currentStage == stage._110_Drive_Out){
            robot.driveTrain.cmdDriveByGyro(AUTO_DRIVE_NORMAL_SPEED, 90, 30);
        ...
        if (currentStage == stage._120_Turn){
            if (robot.driveTrain.getcmdComplete()) {
                robot.driveTrain.cmdTurnByGyro(AUTO_TURN_SPEED, -AUTO_TURN_SPEED, -175);
        ...
        if(currentStage == stage._130_Back_Up){
            if(robot.driveTrain.getcmdComplete()) {
                robot.driveTrain.cmdDriveByGyro(AUTO_DRIVE_NORMAL_SPEED, -175, -18);
